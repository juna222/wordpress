...
function rara_academic_single_post_schema() {
    if ( is_singular( 'post' ) ) {
        ...
        $args = array(
            ...
            "headline"      => esc_url( get_the_title( $post->ID ) ),
            "datePublished" => esc_url( get_the_time( DATE_ISO8601, $post->ID ) ),
            "dateModified"  => esc_url( get_post_modified_time(  DATE_ISO8601, __return_false(), $post->ID ) ),
            "author"        => array(
                "@type"     => "Person",
